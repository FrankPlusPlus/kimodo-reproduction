...
|---|---:|---:|---:|---:|---:|
| 官方 checkpoint | `[待填]` | `[待填]` | `[待填]` | `[待填]` | 未公开/已知值 |
| 随机初始化或早期基线 | `[待填]` | `[待填]` | `[待填]` | `[待填]` | `[待填]` |
| Phase 1 checkpoint | `[待填]` | `[待填]` | `[待填]` | `[待填]` | `[待填]` |
| Phase 2 checkpoint | `[待填]` | `[待填]` | `[待填]` | `[待填]` | `[待填]` |
| 与官方差距 | `[待填]` | `[待填]` | `[待填]` | `[待填]` | — |
...
- 对收敛速度、震荡、过拟合和 plateau 给出解释。
- 将 benchmark 随 checkpoint step 的变化与训练 loss 放在一起分析。
- 检查不同 rank、不同 seed 或重复实验的稳定性。
...
- 哪些指标已接近官方模型，哪些仍存在显著差距。
- 哪些差距可以由数据规模或公开信息缺失解释，哪些仍需实验确认。
- 当前结果是否足以证明训练链路正确，是否足以证明效果复现，分别回答。
- 主动列出负面结果和不确定性，不只展示最佳案例。
