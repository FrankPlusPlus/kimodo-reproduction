...
# Start:
#   exec bash /home/share/yzt/kimodo-reproduction/scripts/eval_lastwd05_from700k_stratified_watcher.sh
set -euo pipefail
...
export KIMODO_TRAIN_RUN_DIR="${KIMODO_TRAIN_RUN_DIR:-${KIMODO_STORAGE_ROOT}/runs/v2-1m-hostnet-lastwd05-from700k}"
export KIMODO_EVAL_EXPORT_RUN_DIR="${KIMODO_EVAL_EXPORT_RUN_DIR:-${KIMODO_STORAGE_ROOT}/eval-exports/v2-1m-hostnet-lastwd05-from700k}"
export KIMODO_RUN_DIR="${KIMODO_RUN_DIR:-${KIMODO_EVAL_EXPORT_RUN_DIR}}"
export KIMODO_EVAL_ROOT="${KIMODO_EVAL_ROOT:-${KIMODO_STORAGE_ROOT}/eval-results/v2-1m-hostnet-lastwd05-from700k-stratified10pct}"
...
if [[ ! -d "${KIMODO_EVAL_TEXT_GALLERY}" ]]; then
  echo "missing frozen text gallery: ${KIMODO_EVAL_TEXT_GALLERY}" >&2
  exit 2
fi
...
if [[ "${KIMODO_EVAL_EXPORT_SIDECAR:-1}" == 1 ]]; then
  ...
  echo "lastwd05-from700k eval: export sidecar pid=$!"
fi

cd "${KIMODO_CODE_ROOT}"
echo "lastwd05-from700k eval: train=${KIMODO_TRAIN_RUN_DIR}"
echo "lastwd05-from700k eval: exports=${KIMODO_RUN_DIR}/exports"
echo "lastwd05-from700k eval: output=${KIMODO_EVAL_ROOT}"
echo "lastwd05-from700k eval: benchmark=${KIMODO_BENCHMARK_ROOT}"
echo "lastwd05-from700k eval: text_gallery=${KIMODO_EVAL_TEXT_GALLERY}"
echo "lastwd05-from700k eval: minimum_step=${KIMODO_EVAL_MINIMUM_STEP} every=${KIMODO_EXPORT_STEP_EVERY}"
