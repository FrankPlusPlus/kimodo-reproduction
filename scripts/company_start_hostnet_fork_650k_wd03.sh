#!/usr/bin/env bash
# Fork kf-smooth 650k into a new stable-reproduction lineage: wd=0.3, Adam
# moment reset, 2k warmup, LR decay to 2e-6 by 1M. Curriculum still ramps to
# Kmax=20. Do not paste K7/800/695k launchers.
#
...
# Start box is sh:
#   exec bash /home/share/yzt/kimodo-reproduction/scripts/company_start_hostnet_fork_650k_wd03.sh
set -euo pipefail
...
export KIMODO_TRAINING_OVERLAY="${KIMODO_TRAINING_OVERLAY:-${KIMODO_CODE_ROOT}/configs/overlays/v2_1m_wd03_from650k.yaml}"
...
PARENT_CHECKPOINT="${KIMODO_FORK_PARENT_CHECKPOINT:-${KIMODO_STORAGE_ROOT}/runs/v2-1m-hostnet-kf-smooth-lr1e5/checkpoints/step-000650000.pt}"
...
log "Kimodo 650k wd=0.3 stable fork: code=${KIMODO_CODE_ROOT} run=${KIMODO_RUN_DIR}"
...
if [ ! -f "${PARENT_CHECKPOINT}" ]; then
  echo "650k parent checkpoint is missing: ${PARENT_CHECKPOINT}" >&2
  exit 2
fi
