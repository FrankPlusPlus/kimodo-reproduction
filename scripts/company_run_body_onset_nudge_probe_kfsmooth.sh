#!/usr/bin/env bash
# Onset causal probe: freeze weights, rotate last-layer attn/FFN cosine by
# ±0.25, read the 7-term training loss. Forward only. No optimizer step.
#
# Checkpoints are the line that actually blew up near 696.6k:
#   kf-smooth 650k (still additive) / 690k (pre-flip, +0.22) / 695k (just flipped)
# Do NOT use wd03 700k or rescue 800k for this question.
# Prefer 1 instance x 1 GPU. Do not Restart. Do not use kimodo-dev.
# Start box is sh:
#   exec bash /home/share/yzt/kimodo-reproduction/scripts/company_run_body_onset_nudge_probe_kfsmooth.sh
set -euo pipefail
...
out_dir="${KIMODO_PROBE_OUT_DIR:-${KIMODO_STORAGE_ROOT}/diagnostics/body-onset-nudge-kfsmooth-690-695}"
...
if ! grep -q "summarize_onset_nudge" "${KIMODO_CODE_ROOT}/kimodo/training/body_onset_nudge_probe.py"; then
  echo "body_onset_nudge_probe.py is missing summarize_onset_nudge; PVC code is stale" >&2
  exit 2
fi
if ! grep -q "preflip-step" "${KIMODO_CODE_ROOT}/scripts/diagnose_body_onset_nudge.py"; then
  echo "diagnose_body_onset_nudge.py is missing --preflip-step; PVC code is stale" >&2
  exit 2
fi
...
echo "kf-smooth onset-nudge probe: python=${python_bin}"
echo "config=${config}"
echo "ckpts=${ckpt_650} ${ckpt_690} ${ckpt_695}"
echo "out=${out_dir}"

diagnose_args=(
  "${KIMODO_CODE_ROOT}/scripts/diagnose_body_onset_nudge.py"
  --config "${config}"
  ...
  --preflip-step 690000
  --flipped-step 695000
  --cosine-delta "${KIMODO_PROBE_COSINE_DELTA:-0.25}"
