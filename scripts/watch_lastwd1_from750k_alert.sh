#!/usr/bin/env bash
# 开发机常驻：750k last-layer wd=1 fork 的 loss 异常 + 每 10k 梯度健康。
# 测评邮件由 watch_lastwd1_from750k_eval_alert.sh 单独发（760k 起每 20k）。
#
#   nohup bash /home/share/yzt/kimodo-reproduction/scripts/watch_lastwd1_from750k_alert.sh \
#     >> /home/share/yzt/kimodo-reproduction/watch/v2-1m-hostnet-lastwd1-from750k/nohup.log 2>&1 &
set -euo pipefail
...
export KIMODO_WATCH_DIR="${KIMODO_WATCH_DIR:-${KIMODO_CODE_ROOT}/watch/v2-1m-hostnet-lastwd1-from750k}"
export KIMODO_HEALTH_10K_START="${KIMODO_HEALTH_10K_START:-760000}"
...
echo "lastwd1-from750k daemon: run=${KIMODO_RUN_DIR}"
echo "lastwd1-from750k daemon: eval=${KIMODO_EVAL_ROOT}"
echo "lastwd1-from750k daemon: health=${KIMODO_HEALTH_10K_START}+${KIMODO_HEALTH_10K_EVERY}"
echo "lastwd1-from750k daemon: train_watch=${KIMODO_TRAIN_WATCH} eval_watch=${KIMODO_EVAL_WATCH} head_to_head=${KIMODO_HEAD_TO_HEAD}"
echo "lastwd1-from750k daemon: watch=${WATCH_DIR}"
echo "lastwd1-from750k daemon: mail=${KIMODO_ALERT_EMAIL}"
