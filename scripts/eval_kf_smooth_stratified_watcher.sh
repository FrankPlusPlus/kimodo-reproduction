#!/usr/bin/env bash
# 1xH200 eval pod for the 600k kf-smooth fork.
# 50k cadence: sidecar exports EMA from 5k checkpoints; this watcher scores
# stratified-10pct into a jovyan-writable results dir.
#
# UI: 1 instance x 1 GPU. Do not attach this to the 16-GPU training job.
# Start:
#   exec bash /home/share/yzt/kimodo-reproduction/scripts/eval_kf_smooth_stratified_watcher.sh
set -euo pipefail
...
# Trainer only auto-exports every 100k. 50k scores come from a sidecar that
# offline-exports EMA bundles from the 5k checkpoints into this writable dir.
export KIMODO_EVAL_EXPORT_RUN_DIR="${KIMODO_EVAL_EXPORT_RUN_DIR:-${KIMODO_STORAGE_ROOT}/eval-exports/v2-1m-hostnet-kf-smooth-lr1e5}"
...
export KIMODO_EXPORT_MIN_STEP="${KIMODO_EXPORT_MIN_STEP:-650000}"
export KIMODO_EXPORT_STEP_EVERY="${KIMODO_EXPORT_STEP_EVERY:-50000}"
export KIMODO_EVAL_MINIMUM_STEP="${KIMODO_EVAL_MINIMUM_STEP:-650000}"
...
if [[ ! -r "${KIMODO_TRAIN_RUN_DIR}/exports/step-000600000/model.pt" ]]; then
  echo "cannot read trainer export: ${KIMODO_TRAIN_RUN_DIR}/exports/step-000600000/model.pt" >&2
  exit 2
fi
...
if [[ "${KIMODO_EVAL_EXPORT_SIDECAR:-1}" == 1 ]]; then
  ...
  echo "kf-smooth eval: export sidecar pid=$!"
fi

cd "${KIMODO_CODE_ROOT}"
echo "kf-smooth eval: train=${KIMODO_TRAIN_RUN_DIR}"
echo "kf-smooth eval: exports=${KIMODO_RUN_DIR}/exports"
echo "kf-smooth eval: output=${KIMODO_EVAL_ROOT}"
echo "kf-smooth eval: benchmark=${KIMODO_BENCHMARK_ROOT}"
echo "kf-smooth eval: minimum_step=${KIMODO_EVAL_MINIMUM_STEP} every=${KIMODO_EXPORT_STEP_EVERY}"
