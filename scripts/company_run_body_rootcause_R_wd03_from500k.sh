#!/usr/bin/env bash
# Experiment R variant: full-stack attn/ffn for 500k wd03 fork timeline.
# Parent 500k + fork 510/520/530/540/550k. Prefer 1x GPU (kimodo-dev OK).
#   exec bash /home/share/yzt/kimodo-reproduction/scripts/company_run_body_rootcause_R_wd03_from500k.sh
set -euo pipefail
...
out_dir="${KIMODO_PROBE_OUT_DIR:-${KIMODO_STORAGE_ROOT}/diagnostics/body-rootcause-R-wd03-from500k}"

ckpt_500="${KIMODO_CKPT_500:-${hostnet_run}/checkpoints/step-000500000.pt}"
ckpt_510="${KIMODO_CKPT_510:-${fork_run}/checkpoints/step-000510000.pt}"
ckpt_520="${KIMODO_CKPT_520:-${fork_run}/checkpoints/step-000520000.pt}"
ckpt_530="${KIMODO_CKPT_530:-${fork_run}/checkpoints/step-000530000.pt}"
...
for path in "${config}" "${ckpt_500}" "${ckpt_510}" "${ckpt_520}" "${ckpt_530}" "${ckpt_540}" "${ckpt_550}"; do
  if [[ ! -r "${path}" ]]; then
...
echo "rootcause R wd03-from500k: out=${out_dir}"
exec "${python_bin}" "${KIMODO_CODE_ROOT}/scripts/diagnose_body_residual_cancel.py" \
  --config "${config}" \
  --checkpoints "${ckpt_500}" "${ckpt_510}" "${ckpt_520}" "${ckpt_530}" "${ckpt_540}" "${ckpt_550}" \
