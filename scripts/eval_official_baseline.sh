...
python benchmark/generate_eval.py \
  ...
  "${text_encoder_args[@]}"
python benchmark/embed_folder.py "${output_root}" --device "${device}" "${text_encoder_args[@]}"
evaluate_args=("${output_root}" --device "${device}")
if [[ "${KIMODO_EVAL_PAPER_PROTOCOL:-0}" == 1 ]]; then
  evaluate_args+=(--paper-protocol)
fi
python benchmark/evaluate_folder.py "${evaluate_args[@]}"
python benchmark/parse_folder.py \
  "${output_root}" \
