...
# new directory. Freezes sampled Kmax at 7 (does not rescale the 1→20 ramp).
#
...
# Do not paste company_start_hostnet_fork_695k.sh (that is the 800-budget
# child dir) or company_start_hostnet_fork_600k.sh / company_start_hostnet_resume.sh.
#
...
#   exec bash /home/share/yzt/kimodo-reproduction/scripts/company_start_hostnet_fork_695k_k7.sh
set -euo pipefail
...
export KIMODO_RUN_DIR="${KIMODO_RUN_DIR:-${KIMODO_STORAGE_ROOT}/runs/v2-1m-hostnet-k7-from695k}"
...
log "Kimodo 695k K7 freeze fork: code=${KIMODO_CODE_ROOT} run=${KIMODO_RUN_DIR}"
