...
# 另外盯 Official SEED-v1.1 与 695k 两个在跑测评，齐了发 MiMo 全面对照。
# 电脑可以带走。SMTP 授权码可稍后写入 PVC .env，进程会自行加载。
#
#   nohup bash /home/share/yzt/kimodo-reproduction/scripts/watch_train_loss_alert.sh \
#     >> /home/share/yzt/kimodo-reproduction/watch/v2-1m-hostnet-cap800-from695k/nohup.log 2>&1 &
set -euo pipefail
...
export KIMODO_EVAL_ROOT="${KIMODO_EVAL_ROOT:-${KIMODO_STORAGE_ROOT}/eval-results/v2-1m-hostnet-cap800-from695k-stratified10pct}"
...
export KIMODO_OFFICIAL_EVAL_SUMMARY="${KIMODO_OFFICIAL_EVAL_SUMMARY:-${KIMODO_STORAGE_ROOT}/eval-results/official-seed-v1-stratified10pct/summary_rows.json}"
export KIMODO_695K_EVAL_SUMMARY="${KIMODO_695K_EVAL_SUMMARY:-${KIMODO_STORAGE_ROOT}/eval-results/v2-1m-hostnet-kf-smooth-lr1e5-step695k-stratified10pct/step-000695000/summary_rows.json}"
export KIMODO_WATCH_DIR="${KIMODO_WATCH_DIR:-${KIMODO_CODE_ROOT}/watch/v2-1m-hostnet-cap800-from695k}"
...
echo "kf-smooth daemon: official=${KIMODO_OFFICIAL_EVAL_SUMMARY}"
echo "kf-smooth daemon: 695k=${KIMODO_695K_EVAL_SUMMARY}"
