...
foundation_model="${KIMODO_LLM2VEC_FOUNDATION:-/home/yezitao/data/yzt/kimodo-repro/models/llm2vec/foundation}"
mntp_model="${KIMODO_LLM2VEC_MNTP:-/home/yezitao/data/yzt/kimodo-repro/models/llm2vec/mntp-adapter}"
supervised_model="${KIMODO_LLM2VEC_SUPERVISED:-/home/yezitao/data/yzt/kimodo-repro/models/llm2vec/supervised-adapter}"
expert_model="${KIMODO_EXPERT_MODEL:-/home/yezitao/data/yzt/kimodo-repro/models/Qwen3-32B}"
...
while [[ ! -f "${responses}" ]]; do
  ...
  "${repo_root}/scripts/build_v2_llm.sh" generate 2>&1 | tee -a "${log_file}"
done
...
if [[ ! -f "${raw_manifest}" ]]; then
  ...
  "${repo_root}/scripts/build_v2_llm.sh" manifest 2>&1 | tee -a "${log_file}"
fi
