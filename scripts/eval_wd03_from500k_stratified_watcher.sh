#!/usr/bin/env bash
# 1xH200 stratified-10pct watcher for the 500k wd=0.3 lr=2e-5 ceiling fork.
# 20k cadence from 520k to catch Phase2 drift vs parent 500k text early.
#
# UI: 1 instance x 1 GPU. Do not attach this to the 16-GPU training job.
# Start:
#   exec bash /home/share/yzt/kimodo-reproduction/scripts/eval_wd03_from500k_stratified_watcher.sh
set -euo pipefail
...
export KIMODO_TRAIN_RUN_DIR="${KIMODO_TRAIN_RUN_DIR:-${KIMODO_STORAGE_ROOT}/runs/v2-1m-hostnet-wd03-from500k}"
export KIMODO_EVAL_EXPORT_RUN_DIR="${KIMODO_EVAL_EXPORT_RUN_DIR:-${KIMODO_STORAGE_ROOT}/eval-exports/v2-1m-hostnet-wd03-from500k}"
...
export KIMODO_EXPORT_MIN_STEP="${KIMODO_EXPORT_MIN_STEP:-520000}"
export KIMODO_EXPORT_STEP_EVERY="${KIMODO_EXPORT_STEP_EVERY:-20000}"
export KIMODO_EVAL_MINIMUM_STEP="${KIMODO_EVAL_MINIMUM_STEP:-520000}"
...
if [[ "${KIMODO_EVAL_EXPORT_SIDECAR:-1}" == 1 ]]; then
  ...
  echo "wd03-from500k eval: export sidecar pid=$!"
fi

cd "${KIMODO_CODE_ROOT}"
echo "wd03-from500k eval: train=${KIMODO_TRAIN_RUN_DIR}"
echo "wd03-from500k eval: exports=${KIMODO_RUN_DIR}/exports"
echo "wd03-from500k eval: output=${KIMODO_EVAL_ROOT}"
echo "wd03-from500k eval: benchmark=${KIMODO_BENCHMARK_ROOT}"
echo "wd03-from500k eval: prior=${KIMODO_PRIOR_EVAL_ROOT}"
echo "wd03-from500k eval: minimum_step=${KIMODO_EVAL_MINIMUM_STEP} every=${KIMODO_EXPORT_STEP_EVERY}"
