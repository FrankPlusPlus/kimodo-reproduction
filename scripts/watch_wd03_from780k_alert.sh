#!/usr/bin/env bash
# 开发机常驻：780k wd=0.3 lr=3e-6 fork 的 loss 异常 + 每 10k 梯度健康。
# 测评邮件由 watch_wd03_from780k_eval_alert.sh 单独发。
#
#   nohup bash /home/share/yzt/kimodo-reproduction/scripts/watch_wd03_from780k_alert.sh \
#     >> /home/share/yzt/kimodo-reproduction/watch/v2-1m-hostnet-wd03-from780k-lr3e6/nohup.log 2>&1 &
set -euo pipefail
...
export KIMODO_WATCH_DIR="${KIMODO_WATCH_DIR:-${KIMODO_CODE_ROOT}/watch/v2-1m-hostnet-wd03-from780k-lr3e6}"
export KIMODO_HEALTH_10K_START="${KIMODO_HEALTH_10K_START:-790000}"
...
echo "wd03-from780k-lr3e6 daemon: run=${KIMODO_RUN_DIR}"
echo "wd03-from780k-lr3e6 daemon: eval=${KIMODO_EVAL_ROOT}"
echo "wd03-from780k-lr3e6 daemon: health=${KIMODO_HEALTH_10K_START}+${KIMODO_HEALTH_10K_EVERY}"
echo "wd03-from780k-lr3e6 daemon: train_watch=${KIMODO_TRAIN_WATCH} eval_watch=${KIMODO_EVAL_WATCH} head_to_head=${KIMODO_HEAD_TO_HEAD}"
echo "wd03-from780k-lr3e6 daemon: watch=${WATCH_DIR}"
echo "wd03-from780k-lr3e6 daemon: mail=${KIMODO_ALERT_EMAIL}"
