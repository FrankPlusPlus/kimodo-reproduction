#!/usr/bin/env bash
# Fork k7-from690k at 696k with a new runtime.seed. Keeps Kmax hard_cap 7.
# Stops at 698k. Do not paste the 690k/695k/800 launchers.
#
# Stop the running k7-from690k 16-GPU job first (same cluster slot), then
# CREATE a new 2x8 task. Do not click Restart.
#
# UI: enable_host_network=True, RDMA/IB x8, 2 instances x 8 GPU.
#   exec bash /home/share/yzt/kimodo-reproduction/scripts/company_start_hostnet_fork_696k_k7_reseed.sh
set -euo pipefail
...
export KIMODO_RUN_DIR="${KIMODO_RUN_DIR:-${KIMODO_STORAGE_ROOT}/runs/v2-1m-hostnet-k7-reseed696k}"
export KIMODO_TRAINING_OVERLAY="${KIMODO_TRAINING_OVERLAY:-${KIMODO_CODE_ROOT}/configs/overlays/v2_1m_k7_reseed696k.yaml}"
...
PARENT_CHECKPOINT="${KIMODO_FORK_PARENT_CHECKPOINT:-${KIMODO_STORAGE_ROOT}/runs/v2-1m-hostnet-k7-from690k/checkpoints/step-000696000.pt}"
PARENT_LOCK="${KIMODO_STORAGE_ROOT}/runs/v2-1m-hostnet-k7-from690k/.kimodo-active-run.lock"
...
log "Kimodo 696k K7 reseed fork: code=${KIMODO_CODE_ROOT} run=${KIMODO_RUN_DIR}"
...
if ! grep -qE "seed:[[:space:]]*4321" "${KIMODO_TRAINING_OVERLAY}"; then
  echo "overlay must set runtime.seed 4321 to desync data/noise from seed 1234: ${KIMODO_TRAINING_OVERLAY}" >&2
  exit 2
fi
if ! grep -q "max_steps_override: 698000" "${KIMODO_TRAINING_OVERLAY}"; then
  echo "overlay must stop at 698000 for the 2k-step diagnostic: ${KIMODO_TRAINING_OVERLAY}" >&2
  exit 2
fi
...
if [ ! -f "${PARENT_CHECKPOINT}" ]; then
  echo "696k K7 parent checkpoint is missing: ${PARENT_CHECKPOINT}" >&2
  exit 2
fi
