#!/usr/bin/env bash
# 1xH200 stratified-10pct watcher for the 780k wd=0.3 lr=3e-6 rescue fork.
# 50k cadence from 800k (first quality gate after the 20k jsonl look).
...
# Start:
#   exec bash /home/share/yzt/kimodo-reproduction/scripts/eval_wd03_from780k_stratified_watcher.sh
set -euo pipefail
...
export KIMODO_EVAL_EXPORT_RUN_DIR="${KIMODO_EVAL_EXPORT_RUN_DIR:-${KIMODO_STORAGE_ROOT}/eval-exports/v2-1m-hostnet-wd03-from780k-lr3e6}"
...
export KIMODO_EXPORT_MIN_STEP="${KIMODO_EXPORT_MIN_STEP:-800000}"
export KIMODO_EXPORT_STEP_EVERY="${KIMODO_EXPORT_STEP_EVERY:-50000}"
export KIMODO_EVAL_MINIMUM_STEP="${KIMODO_EVAL_MINIMUM_STEP:-800000}"
...
if [[ "${KIMODO_EVAL_EXPORT_SIDECAR:-1}" == 1 ]]; then
  ...
  echo "wd03-from780k-lr3e6 eval: export sidecar pid=$!"
fi

cd "${KIMODO_CODE_ROOT}"
echo "wd03-from780k-lr3e6 eval: train=${KIMODO_TRAIN_RUN_DIR}"
echo "wd03-from780k-lr3e6 eval: exports=${KIMODO_RUN_DIR}/exports"
echo "wd03-from780k-lr3e6 eval: output=${KIMODO_EVAL_ROOT}"
echo "wd03-from780k-lr3e6 eval: benchmark=${KIMODO_BENCHMARK_ROOT}"
echo "wd03-from780k-lr3e6 eval: minimum_step=${KIMODO_EVAL_MINIMUM_STEP} every=${KIMODO_EXPORT_STEP_EVERY}"
