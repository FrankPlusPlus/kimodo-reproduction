#!/usr/bin/env bash
# 1xH200 stratified-10pct watcher for the 750k last-layer wd=1 fork.
# 20k cadence from 760k. Same pod also photographs last-layer attn cosine
# vs preserved 750k and snapshots jsonl L15/clip (today's drift diagnosis).
...
# Do not use kimodo-dev. Recreate/attach with kimodo-reproduction:v8.
# If an older 50k/800k eval pod is already running, CREATE a new one.
# Start box is sh:
#   exec bash /home/share/yzt/kimodo-reproduction/scripts/eval_lastwd1_from750k_stratified_watcher.sh
set -euo pipefail
...
export KIMODO_TRAIN_RUN_DIR="${KIMODO_TRAIN_RUN_DIR:-${KIMODO_STORAGE_ROOT}/runs/v2-1m-hostnet-lastwd1-from750k}"
export KIMODO_EVAL_EXPORT_RUN_DIR="${KIMODO_EVAL_EXPORT_RUN_DIR:-${KIMODO_STORAGE_ROOT}/eval-exports/v2-1m-hostnet-lastwd1-from750k}"
...
export KIMODO_EXPORT_MIN_STEP="${KIMODO_EXPORT_MIN_STEP:-760000}"
export KIMODO_EXPORT_STEP_EVERY="${KIMODO_EXPORT_STEP_EVERY:-20000}"
export KIMODO_EVAL_MINIMUM_STEP="${KIMODO_EVAL_MINIMUM_STEP:-760000}"
export KIMODO_DRIFT_OUT_DIR="${KIMODO_DRIFT_OUT_DIR:-${KIMODO_STORAGE_ROOT}/diagnostics/lastwd1-from750k-drift}"
export KIMODO_DRIFT_MIN_STEP="${KIMODO_DRIFT_MIN_STEP:-760000}"
export KIMODO_DRIFT_STEP_EVERY="${KIMODO_DRIFT_STEP_EVERY:-20000}"
...
mkdir -p "${KIMODO_EVAL_EXPORT_RUN_DIR}/exports" "${KIMODO_EVAL_ROOT}" "${KIMODO_DRIFT_OUT_DIR}"
...
if [[ "${KIMODO_EVAL_EXPORT_SIDECAR:-1}" == 1 ]]; then
  ...
  echo "lastwd1-from750k eval: export sidecar pid=$!"
fi
if [[ "${KIMODO_DRIFT_SIDECAR:-1}" == 1 ]]; then
  nohup bash "${KIMODO_CODE_ROOT}/scripts/watch_lastwd1_drift.sh" \
    >>"${KIMODO_DRIFT_OUT_DIR}/drift-sidecar.log" 2>&1 &
  echo "lastwd1-from750k eval: drift sidecar pid=$!"
fi

cd "${KIMODO_CODE_ROOT}"
echo "lastwd1-from750k eval: train=${KIMODO_TRAIN_RUN_DIR}"
echo "lastwd1-from750k eval: exports=${KIMODO_RUN_DIR}/exports"
echo "lastwd1-from750k eval: output=${KIMODO_EVAL_ROOT}"
echo "lastwd1-from750k eval: drift=${KIMODO_DRIFT_OUT_DIR}"
echo "lastwd1-from750k eval: benchmark=${KIMODO_BENCHMARK_ROOT}"
echo "lastwd1-from750k eval: minimum_step=${KIMODO_EVAL_MINIMUM_STEP} every=${KIMODO_EXPORT_STEP_EVERY}"
echo "lastwd1-from750k eval: compare to parent 750k FID 0.116 / cosine -0.02 / L15 0.13"
echo "lastwd1-from750k eval: text_gallery=${KIMODO_EVAL_TEXT_GALLERY:-auto-750k}"
