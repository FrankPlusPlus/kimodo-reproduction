#!/usr/bin/env bash
# Fork original v2-1m-hostnet 500k into a ceiling-check lineage:
# wd=0.3, paper lr=2e-5, standard K 1→20 ramp (phase2_steps=250k), cap 750k.
#
...
# Start box is sh:
#   exec bash /home/share/yzt/kimodo-reproduction/scripts/company_start_hostnet_fork_500k_wd03.sh
set -euo pipefail
...
export KIMODO_TRAINING_OVERLAY="${KIMODO_TRAINING_OVERLAY:-${KIMODO_CODE_ROOT}/configs/overlays/v2_1m_wd03_from500k.yaml}"
...
PARENT_CHECKPOINT="${KIMODO_FORK_PARENT_CHECKPOINT:-${KIMODO_STORAGE_ROOT}/runs/v2-1m-hostnet/checkpoints/step-000500000.pt}"
PARENT_LOCK="${KIMODO_STORAGE_ROOT}/runs/v2-1m-hostnet/.kimodo-active-run.lock"
...
log "Kimodo 500k wd=0.3 lr=2e-5 ceiling fork: code=${KIMODO_CODE_ROOT} run=${KIMODO_RUN_DIR}"
...
if ! grep -q "max_steps_override" "${KIMODO_CODE_ROOT}/kimodo/training/config.py"; then
  echo "config.py does not define max_steps_override; PVC code is stale" >&2
  exit 2
fi
...
if ! grep -qE "learning_rate:[[:space:]]*2\.0e-5" "${KIMODO_TRAINING_OVERLAY}"; then
  echo "overlay must keep paper learning_rate: 2.0e-5: ${KIMODO_TRAINING_OVERLAY}" >&2
  exit 2
fi
...
if ! grep -q "max_steps_override: 750000" "${KIMODO_TRAINING_OVERLAY}"; then
  echo "overlay must cap at max_steps_override: 750000: ${KIMODO_TRAINING_OVERLAY}" >&2
  exit 2
fi
...
if ! grep -q "phase2_steps: 250000" "${KIMODO_TRAINING_OVERLAY}"; then
  echo "overlay must set phase2_steps: 250000 so K reaches 20 at 750k: ${KIMODO_TRAINING_OVERLAY}" >&2
  exit 2
fi
if grep -qE "sparse_keyframes_min:[[:space:]]*20[[:space:]]*$" "${KIMODO_TRAINING_OVERLAY}"; then
  echo "overlay must not jump K to 20 at fork (use standard 1→20 ramp)" >&2
  exit 2
fi
...
if grep -qE "lr_end:|lr_schedule_start_step:" "${KIMODO_TRAINING_OVERLAY}"; then
  echo "overlay must keep constant lr=2e-5 (no lr decay schedule)" >&2
  exit 2
fi
...
if [ ! -f "${PARENT_CHECKPOINT}" ]; then
  echo "500k parent checkpoint is missing: ${PARENT_CHECKPOINT}" >&2
  exit 2
fi
