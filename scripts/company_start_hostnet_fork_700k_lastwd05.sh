#!/usr/bin/env bash
# Fork wd03 parent 700k: rest wd=0.3, last body layer wd=0.5, text curriculum bias.
# LR warmup+decay anchored at the fork step (700k → 850k). Adam reset on fork.
# Curriculum still ramps to Kmax=20. Do not paste lastwd1 / lr3e6 launchers.
#
...
# Start box is sh:
#   exec bash /home/share/yzt/kimodo-reproduction/scripts/company_start_hostnet_fork_700k_lastwd05.sh
set -euo pipefail
...
export KIMODO_RUN_DIR="${KIMODO_RUN_DIR:-${KIMODO_STORAGE_ROOT}/runs/v2-1m-hostnet-lastwd05-from700k}"
export KIMODO_TRAINING_OVERLAY="${KIMODO_TRAINING_OVERLAY:-${KIMODO_CODE_ROOT}/configs/overlays/v2_1m_lastwd05_from700k.yaml}"
...
PARENT_CHECKPOINT="${KIMODO_FORK_PARENT_CHECKPOINT:-${KIMODO_STORAGE_ROOT}/runs/v2-1m-hostnet-wd03-from650k/checkpoints/step-000700000.pt}"
...
log "Kimodo 700k last-layer wd=0.5 fork: code=${KIMODO_CODE_ROOT} run=${KIMODO_RUN_DIR}"
...
if ! grep -qE "last_layer_weight_decay:[[:space:]]*0\.5" "${KIMODO_TRAINING_OVERLAY}"; then
  echo "overlay must set last_layer_weight_decay: 0.5: ${KIMODO_TRAINING_OVERLAY}" >&2
  exit 2
fi
...
if ! grep -q "lr_schedule_start_step: 700000" "${KIMODO_TRAINING_OVERLAY}"; then
  echo "overlay must anchor LR schedule at fork step 700000: ${KIMODO_TRAINING_OVERLAY}" >&2
  exit 2
fi
...
if grep -qE "learning_rate:[[:space:]]*2\.0e-5" "${KIMODO_TRAINING_OVERLAY}"; then
  echo "overlay must not raise LR back to Phase 1 2e-5 at 700k onset" >&2
  exit 2
fi
...
if grep -qE "last_layer_weight_decay:[[:space:]]*1\.0" "${KIMODO_TRAINING_OVERLAY}"; then
  echo "overlay must not reuse lastwd1@1.0" >&2
  exit 2
fi
...
if [ ! -f "${PARENT_CHECKPOINT}" ]; then
  echo "700k parent checkpoint is missing: ${PARENT_CHECKPOINT}" >&2
  exit 2
fi
