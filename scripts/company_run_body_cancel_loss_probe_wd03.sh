#!/usr/bin/env bash
# Experiment 2: freeze weights, peel the anti-parallel part of last-layer
# attn (and FFN) so x and the branch cancel less, then read the 7-term loss.
# Forward only. No optimizer step, no weight write.
#
# Checkpoints: parent 750k (control) and rescue 800k (already cancelling).
# Prefer 1 instance x 1 GPU. Do not Restart. Do not use kimodo-dev.
# Start box is sh:
#   exec bash /home/share/yzt/kimodo-reproduction/scripts/company_run_body_cancel_loss_probe_wd03.sh
set -euo pipefail
...
out_dir="${KIMODO_PROBE_OUT_DIR:-${KIMODO_STORAGE_ROOT}/diagnostics/body-cancel-loss-wd03}"
...
if ! grep -q "summarize_cancel_loss" "${KIMODO_CODE_ROOT}/kimodo/training/body_cancel_loss_probe.py"; then
  echo "body_cancel_loss_probe.py is missing summarize_cancel_loss; PVC code is stale" >&2
  exit 2
fi
if ! grep -q "takeoff-step" "${KIMODO_CODE_ROOT}/scripts/diagnose_body_cancel_loss.py"; then
  echo "diagnose_body_cancel_loss.py is missing --takeoff-step; PVC code is stale" >&2
  exit 2
fi
...
for path in "${config}" "${ckpt_750}" "${ckpt_800}"; do
  if [[ ! -r "${path}" ]]; then
...
echo "wd03 cancel-loss probe: python=${python_bin}"
echo "config=${config}"
echo "ckpts=${ckpt_750} ${ckpt_800}"
echo "out=${out_dir}"

diagnose_args=(
  "${KIMODO_CODE_ROOT}/scripts/diagnose_body_cancel_loss.py"
  --config "${config}"
  --checkpoints "${ckpt_750}" "${ckpt_800}"
