#!/bin/sh
# Fork the collapsed V2 1M hostnet run from the last eval-healthy 600k milestone.
#
# Stop every previous v2-1m job first, then CREATE a new 2x8 task.
...
# Do not paste company_start_hostnet_resume.sh (that continues collapsed 720k).
#
# UI: enable_host_network=True, RDMA/IB x8, 2 instances x 8 GPU.
# The platform start box is /bin/sh. Prefer this single command there:
#   exec /bin/sh /home/share/yzt/kimodo-reproduction/scripts/company_start_hostnet_fork_600k.sh
set -eu
...
PARENT_CHECKPOINT="${KIMODO_FORK_PARENT_CHECKPOINT:-${KIMODO_STORAGE_ROOT}/runs/v2-1m-hostnet/checkpoints/step-000600000.pt}"
...
log "Kimodo 600k fork: code=${KIMODO_CODE_ROOT} run=${KIMODO_RUN_DIR}"
...
if [ -f "${PARENT_LOCK}" ]; then
  log "warning: parent run lock exists at ${PARENT_LOCK}; the immutable 600k checkpoint can still be forked, but confirm the old GPU job is stopped"
fi
...
if [ ! -f "${PARENT_CHECKPOINT}" ]; then
  echo "600k parent checkpoint is missing: ${PARENT_CHECKPOINT}" >&2
  exit 2
fi
...
# Stage the 1.4G manifest once per Pod. Relative asset paths still resolve
# against the logical PVC path; startup skips per-file NFS stats.
...
local_manifest_read="$("${KIMODO_PYTHON:-python}" "${STAGE_HELPER}" "${MANIFEST_PATH}" "${KIMODO_STARTUP_CACHE_ROOT}" --fallback-root "${local_cache_fallback}")"
