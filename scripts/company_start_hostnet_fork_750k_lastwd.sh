#!/usr/bin/env bash
# Fork preserved wd03 750k: rest wd=0.3, last body layer wd=1.0, Adam reset.
# Keep the parent LR schedule (peak 1e-5 from 650k, decay to 2e-6 by 1M).
...
# Start box is sh:
#   exec bash /home/share/yzt/kimodo-reproduction/scripts/company_start_hostnet_fork_750k_lastwd.sh
set -euo pipefail
...
export KIMODO_RUN_DIR="${KIMODO_RUN_DIR:-${KIMODO_STORAGE_ROOT}/runs/v2-1m-hostnet-lastwd1-from750k}"
export KIMODO_TRAINING_OVERLAY="${KIMODO_TRAINING_OVERLAY:-${KIMODO_CODE_ROOT}/configs/overlays/v2_1m_lastwd1_from750k.yaml}"
...
preserve_ckpt="${KIMODO_STORAGE_ROOT}/preserved-pre-collapse/v2-1m-hostnet-wd03-from650k/step-000750000.pt"
run_ckpt="${KIMODO_STORAGE_ROOT}/runs/v2-1m-hostnet-wd03-from650k/checkpoints/step-000750000.pt"
if [[ -n "${KIMODO_FORK_PARENT_CHECKPOINT:-}" ]]; then
  PARENT_CHECKPOINT="${KIMODO_FORK_PARENT_CHECKPOINT}"
elif [[ -r "${preserve_ckpt}" ]]; then
  PARENT_CHECKPOINT="${preserve_ckpt}"
else
  PARENT_CHECKPOINT="${run_ckpt}"
fi
...
log "Kimodo 750k last-layer wd=1 fork: code=${KIMODO_CODE_ROOT} run=${KIMODO_RUN_DIR}"
...
if ! grep -q "last_layer_weight_decay" "${KIMODO_CODE_ROOT}/kimodo/training/config.py"; then
  echo "config.py does not define last_layer_weight_decay; PVC code is stale" >&2
  exit 2
fi
if ! grep -q "_body_last_layer_parameters" "${KIMODO_CODE_ROOT}/kimodo/training/optim.py"; then
  echo "optim.py is missing last-layer param groups; PVC code is stale" >&2
  exit 2
fi
if ! grep -q "_weight_decay_for_group" "${KIMODO_CODE_ROOT}/kimodo/training/engine.py"; then
  echo "engine.py still clobbers last-layer weight decay; PVC code is stale" >&2
  exit 2
fi
...
if ! grep -qE "last_layer_weight_decay:[[:space:]]*1\.0" "${KIMODO_TRAINING_OVERLAY}"; then
  echo "overlay must set last_layer_weight_decay: 1.0: ${KIMODO_TRAINING_OVERLAY}" >&2
  exit 2
fi
...
if ! grep -q "lr_schedule_start_step: 650000" "${KIMODO_TRAINING_OVERLAY}"; then
  echo "overlay must keep the parent LR schedule start at 650000: ${KIMODO_TRAINING_OVERLAY}" >&2
  exit 2
fi
if grep -qE "learning_rate:[[:space:]]*2\.0e-5" "${KIMODO_TRAINING_OVERLAY}"; then
  echo "overlay must not raise LR back to Phase 1 2e-5 at 750k onset" >&2
  exit 2
fi
if grep -qE "learning_rate:[[:space:]]*3\.0e-6" "${KIMODO_TRAINING_OVERLAY}"; then
  echo "overlay must not reuse the 780k lr=3e-6 rescue" >&2
  exit 2
fi
...
if [ ! -f "${PARENT_CHECKPOINT}" ]; then
  echo "750k parent checkpoint is missing: ${PARENT_CHECKPOINT}" >&2
  exit 2
fi
