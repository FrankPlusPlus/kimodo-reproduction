#!/usr/bin/env bash
# Fork the kf-smooth V2 1M run from the last healthy 695k checkpoint into a
# new directory. Uses the 800-channel affordable sampler overlay.
#
...
# Do not paste company_start_hostnet_fork_600k.sh (that AUTO_RESUMEs the
# dead kf-smooth run dir) or company_start_hostnet_resume.sh.
#
...
#   exec bash /home/share/yzt/kimodo-reproduction/scripts/company_start_hostnet_fork_695k.sh
set -euo pipefail
...
export KIMODO_RUN_DIR="${KIMODO_RUN_DIR:-${KIMODO_STORAGE_ROOT}/runs/v2-1m-hostnet-cap800-from695k}"
export KIMODO_TRAINING_OVERLAY="${KIMODO_TRAINING_OVERLAY:-${KIMODO_CODE_ROOT}/configs/overlays/v2_1m_from600k_kf_smooth.yaml}"
...
PARENT_CHECKPOINT="${KIMODO_FORK_PARENT_CHECKPOINT:-${KIMODO_STORAGE_ROOT}/runs/v2-1m-hostnet-kf-smooth-lr1e5/checkpoints/step-000695000.pt}"
...
log "Kimodo 695k cap800 fork: code=${KIMODO_CODE_ROOT} run=${KIMODO_RUN_DIR}"
...
if ! grep -q "sparse_channel_budget" "${KIMODO_CODE_ROOT}/kimodo/training/config.py"; then
  echo "config.py does not define sparse_channel_budget; PVC code is stale" >&2
  exit 2
fi
if ! grep -q "sparse_channel_budget:" "${KIMODO_TRAINING_OVERLAY}"; then
  echo "overlay is missing sparse_channel_budget: ${KIMODO_TRAINING_OVERLAY}" >&2
  exit 2
fi
if ! grep -q "_prepare_affordable_budget" "${KIMODO_CODE_ROOT}/kimodo/training/constraints.py"; then
  echo "constraints.py is missing affordable sampling; PVC code is stale" >&2
  exit 2
fi
...
if [ ! -f "${PARENT_CHECKPOINT}" ]; then
  echo "695k parent checkpoint is missing: ${PARENT_CHECKPOINT}" >&2
  exit 2
fi
