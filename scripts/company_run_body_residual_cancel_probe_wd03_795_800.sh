#!/usr/bin/env bash
# Read-only residual-cancellation probe on the stopped lr3e6 rescue:
#   795k vs 800k, same frozen batch as the other probes
#   L15: cosine(x, attn(x)), rms(x+attn(x)), LayerNorm σ
# Forward only. No optimizer step, no weight write.
#
# Prefer 1 instance x 1 GPU. Do not Restart. Do not use kimodo-dev.
# Start box is sh:
#   exec bash /home/share/yzt/kimodo-reproduction/scripts/company_run_body_residual_cancel_probe_wd03_795_800.sh
set -euo pipefail
...
out_dir="${KIMODO_PROBE_OUT_DIR:-${KIMODO_STORAGE_ROOT}/diagnostics/body-residual-cancel-wd03-795-800}"
...
if ! grep -q "summarize_residual_cancel" "${KIMODO_CODE_ROOT}/kimodo/training/body_residual_cancel_probe.py"; then
  echo "body_residual_cancel_probe.py is missing summarize_residual_cancel; PVC code is stale" >&2
  exit 2
fi
if ! grep -q "constraint-step" "${KIMODO_CODE_ROOT}/scripts/diagnose_body_residual_cancel.py"; then
  echo "diagnose_body_residual_cancel.py is missing --constraint-step; PVC code is stale" >&2
  exit 2
fi
...
echo "wd03 795/800 residual-cancel probe: python=${python_bin}"
echo "config=${config}"
