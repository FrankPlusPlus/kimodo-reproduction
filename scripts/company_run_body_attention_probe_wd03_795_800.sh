#!/usr/bin/env bash
# Read-only attention pointer probe on the stopped lr3e6 rescue:
...
#
# Prefer 1 instance x 1 GPU. Same frozen-batch recipe as the Jacobian 2x2
# (seed 20260816, 4 samples). Do not Restart a queued job.
# Do not use kimodo-dev. Do not attach to the 795 eval pod.
# Start box is sh:
#   exec bash /home/share/yzt/kimodo-reproduction/scripts/company_run_body_attention_probe_wd03_795_800.sh
set -euo pipefail
...
out_dir="${KIMODO_PROBE_OUT_DIR:-${KIMODO_STORAGE_ROOT}/diagnostics/body-attention-wd03-795-800}"
...
if ! grep -q "summarize_pointer_grid" "${KIMODO_CODE_ROOT}/kimodo/training/body_attention_probe.py"; then
  echo "body_attention_probe.py is missing summarize_pointer_grid; PVC code is stale" >&2
  exit 2
fi
if ! grep -q "constraint-steps" "${KIMODO_CODE_ROOT}/scripts/diagnose_body_attention.py"; then
  echo "diagnose_body_attention.py is missing --constraint-steps; PVC code is stale" >&2
  exit 2
fi
...
if [[ -z "${KIMODO_PROBE_SKIP_TORCHRUN:-}" ]]; then
  if [[ -z "${MASTER_ADDR}" ]]; then
    echo "MASTER_ADDR/PET_MASTER_ADDR is empty; worker cannot join rank0. Create a new 1-GPU job, do not Restart." >&2
...
echo "wd03 795/800 attention probe: python=${python_bin}"
echo "config=${config}"
...
diagnose_args=(
  "${KIMODO_CODE_ROOT}/scripts/diagnose_body_attention.py"
  --config "${config}"
...
if [[ -n "${KIMODO_ATTENTION_LAYERS:-}" ]]; then
  # shellcheck disable=SC2206
  layer_args=(${KIMODO_ATTENTION_LAYERS})
  diagnose_args+=(--layers "${layer_args[@]}")
fi
