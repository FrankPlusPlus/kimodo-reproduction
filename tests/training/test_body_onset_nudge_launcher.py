...
LAUNCHER = PROJECT_ROOT / "scripts/company_run_body_onset_nudge_probe_kfsmooth.sh"


class BodyOnsetNudgeLauncherTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        (scripts / "diagnose_body_onset_nudge.py").write_text(
            "# preflip-step\nprint('probe')\n",
            encoding="utf-8",
        )
        training = self.code_root / "kimodo/training"
        training.mkdir(parents=True)
        (training / "body_onset_nudge_probe.py").write_text(
    ...
    def test_passes_preflip_checkpoints_and_delta(self) -> None:
        diagnose = self.code_root / "scripts/diagnose_body_onset_nudge.py"
        diagnose.write_text(
        ...
        self.assertIn("--flipped-step 695000", result.stdout)
        self.assertIn("--cosine-delta 0.25", result.stdout)
        self.assertIn("body-onset-nudge-kfsmooth-690-695", result.stdout)
        self.assertNotIn("wd03-from650k", result.stdout)
        self.assertNotIn("step-000800000.pt", result.stdout)

    def test_stale_module_fails(self) -> None:
        (self.code_root / "kimodo/training/body_onset_nudge_probe.py").write_text("x = 1\n")
        result = subprocess.run(
            ["/bin/bash", str(self.code_root / "scripts" / LAUNCHER.name)],
            cwd=self.code_root,
            env=self._env(),
        ...
        self.assertIn("summarize_onset_nudge", result.stderr)
