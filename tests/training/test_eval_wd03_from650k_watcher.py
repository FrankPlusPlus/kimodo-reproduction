...
WATCHER = PROJECT_ROOT / "scripts/eval_wd03_from650k_stratified_watcher.sh"


class EvalWd03From650kWatcherTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_points_at_wd03_run_and_700k_cadence(self) -> None:
        ...
        self.assertTrue(run_dir.endswith("eval-exports/v2-1m-hostnet-wd03-from650k"))
        self.assertTrue(
            eval_root.endswith("eval-results/v2-1m-hostnet-wd03-from650k-stratified10pct")
        )
        self.assertTrue(benchmark.endswith("benchmark/stratified-10pct"))
        self.assertEqual(lines[1], "--minimum-step")
        self.assertEqual(lines[2], "700000")
