...
def _load_paper_protocol_outputs(root: Path) -> dict[str, Any]:
    """Load strict Sec. 6.1 artifacts without mixing them into legacy rows.

    Legacy folder-level TMR values were computed within testcase groups and
    cannot be averaged into the paper's full-test-set retrieval/FID.  The
    separately named files are therefore exposed under their own namespace.
    """
    retrieval: list[dict[str, Any]] = []
    constraints: list[dict[str, Any]] = []
    for path in sorted(root.glob("*/text2motion/*/paper_retrieval.json")):
        payload = json.loads(path.read_text(encoding="utf-8"))
        rel = path.relative_to(root).parts
        retrieval.append({"split": rel[0], "category": rel[2], **payload})
    for path in sorted(root.glob("*/paper_constraint_metrics.json")):
        payload = json.loads(path.read_text(encoding="utf-8"))
        constraints.append({"split": path.relative_to(root).parts[0], **payload})
    return {
        "retrieval": retrieval,
        "constraints": constraints,
        "note": (
            "These Sec. 6.1 values are computed from complete sets/raw constraint points; "
            "legacy public-benchmark rows remain separate for backward compatibility."
        ),
    }
...
def _build_summary(root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "rows": rows,
        "tables": tables,
        "paper_protocol": _load_paper_protocol_outputs(root),
    }
