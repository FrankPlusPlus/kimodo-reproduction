...
def _quaternion_slerp(q0: torch.Tensor, q1: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
    ...
    # The analytic SLERP weights are 0/0 when the rotations are identical.
    # Normalized linear interpolation has the same limit and is numerically
    # stable for identical and nearly-identical unit quaternions.
    q_lerp = (1.0 - t) * q0 + t * q1
    q_lerp = q_lerp / torch.linalg.norm(q_lerp, dim=-1, keepdim=True).clamp(min=1e-8)
    ...
    q_slerp = s0 * q0 + s1 * q1
    q_slerp = q_slerp / torch.linalg.norm(q_slerp, dim=-1, keepdim=True).clamp(min=1e-8)
    return torch.where(dot > 1.0 - 1e-6, q_lerp, q_slerp)
...
def resample_motion_dict_to_kimodo_fps(
    ...
    duration as the input, i.e. input frame ``i`` is at ``i / source_fps`` and
    output frame ``j`` is at ``j / target_fps``.  Integer downsampling (for
    example 120 -> 30 Hz) therefore takes every fourth frame.  Other ratios use
    linear interpolation for root translation and quaternion SLERP per joint.
    ...
    # A T-frame sampled signal occupies [0, T / fps).  Using ceil preserves
    # that half-open duration and, importantly, agrees exactly with [::step]
    # for integer downsampling even when T is not divisible by ``step``.
    t_out = max(1, int(np.ceil(t_in * target_fps / source_fps - 1e-12)))
    ...
    step = round(ratio)
    if step >= 2 and abs(ratio - step) < 1e-6:
        local_out = local_rot_mats[::step]
        root_out = root_positions[::step]
        # Guard against floating-point ratio tolerance changing the count.
        # The exact-integer case naturally has exactly ``t_out`` elements.
        local_out = local_out[:t_out]
        root_out = root_out[:t_out]
    else:
        device = local_rot_mats.device
        dtype = local_rot_mats.dtype
        # Do not use linspace(0, T-1): that forces the final input sample onto
        # the final output sample and subtly changes playback speed.  Sampling
        # on the target time grid preserves the declared frame rates.
        u = torch.arange(t_out, device=device, dtype=dtype) * float(ratio)
        u = u.clamp_max(t_in - 1)
...
def load_motion_file(
    ...
    *,
    ...
    # Always delegate a declared FPS difference to the resampler.  Its
    # duration-aware fast path can still avoid interpolation when the target
    # grid is identical, while a fixed tolerance here can silently lose a
    # frame on sufficiently long clips.
    if float(effective_source) != float(target_fps):
