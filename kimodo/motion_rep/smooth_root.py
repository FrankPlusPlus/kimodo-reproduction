...
def smooth_signal(x, margins, pos_weight=0, alpha_overrelax=1.8, admm_iters=500, circle_project=False):
    ...
    # The coarsest multigrid system has no acceleration rows below five
    # frames and is therefore singular.  Returning the input is the exact
    # no-acceleration-prior fallback and keeps short temporal labels usable.
    if len(x) < 5:
        return x.copy()
