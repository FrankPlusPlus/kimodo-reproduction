...
def cont6d_to_matrix(cont6d: torch.Tensor) -> torch.Tensor:
    ...
    # Denoisers can emit a zero vector early in training. Keep FK loss finite
    # for that valid optimizer state rather than dividing by zero.
    x = x_raw / torch.norm(x_raw, dim=-1, keepdim=True).clamp_min(1e-8)
    z = torch.cross(x, y_raw, dim=-1)
    z = z / torch.norm(z, dim=-1, keepdim=True).clamp_min(1e-8)
