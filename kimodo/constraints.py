...
def _validate_frame_indices(frame_indices: Tensor) -> None:
    """Reject negative constraint frames before PyTorch can wrap-index from the end."""
    if frame_indices.ndim != 1:
        raise ValueError("frame_indices must be a one-dimensional tensor")
...
class Root2DConstraintSet:
    ...
    def __init__(
        ...
        _validate_frame_indices(frame_indices)
...
class FootContactConstraintSet:
    """Constraint sparse four-channel foot-contact configurations.

    The paper includes this condition family, while the original public
    authoring classes only covered pose and root constraints.  The feature is
    already part of ``KimodoMotionRep``; this class exposes the missing public
    conversion without changing existing JSON schemas.
    """
    ...
    def __init__(
        ...
        self.frame_indices = frame_indices
        if to_crop:
            foot_contacts = foot_contacts[frame_indices]
        elif len(foot_contacts) != len(frame_indices):
            raise ValueError("foot_contacts and frame_indices must have equal length")
        if foot_contacts.ndim != 2 or foot_contacts.shape[-1] != 4:
            raise ValueError("foot_contacts must have shape [K,4]")
    ...
    def to(
        ...
        self.frame_indices = _tensor_to(self.frame_indices, device=device)
        self.foot_contacts = _tensor_to(self.foot_contacts, device=device, dtype=dtype)
        if device is not None and hasattr(self.skeleton, "to"):
            self.skeleton = self.skeleton.to(device)
        return self
    ...
    def from_dict(cls, skeleton: SkeletonBase, dico: dict) -> "FootContactConstraintSet":
        ...
        return cls(
            ...
            foot_contacts=torch.tensor(dico["foot_contacts"], device=device),
        )
