    ...
    def forward(
        ...
        if self.motion_mask_mode == "concat":
            if motion_mask is None or observed_motion is None:
                motion_mask = torch.zeros_like(x, dtype=torch.bool)
                observed_motion = torch.zeros_like(x)
            motion_mask_bool = motion_mask.bool()
            x = torch.where(motion_mask_bool, observed_motion.to(dtype=x.dtype), x)
            motion_mask_features = motion_mask_bool.to(dtype=x.dtype)
            x_extended = torch.cat([x, motion_mask_features], axis=-1)
        else:
            x_extended = x
        ...
        convert_ctx = (
            torch.no_grad()
            if self.training and self.detach_root_for_body
        ...
        if self.motion_mask_mode == "concat":
            x_new_extended = torch.cat([x_new, motion_mask_features], axis=-1)
        else:
