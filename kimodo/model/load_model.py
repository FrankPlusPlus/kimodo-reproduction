...
def load_checkpoint_bundle(
    ...
    """Load an exported/local Kimodo bundle without model-registry lookup.

    The directory must contain ``config.yaml``, the referenced denoiser
    weights, and normalization statistics. This is the bridge from the
    reconstructed trainer into the released benchmark generation pipeline.
    """
    model_path = Path(checkpoint_dir).expanduser().resolve()
    model_config_path = model_path / "config.yaml"
    if not model_config_path.is_file():
        raise FileNotFoundError(f"Checkpoint bundle is missing config.yaml: {model_config_path}")
    model_conf = OmegaConf.load(model_config_path)
    runtime = {"checkpoint_dir": str(model_path)}
    if text_encoder is None:
        text_encoder_url = get_env_var("TEXT_ENCODER_URL", DEFAULT_TEXT_ENCODER_URL)
        runtime["text_encoder"] = _select_text_encoder_conf(text_encoder_url, text_encoder_fp32)
    else:
        runtime["text_encoder"] = None
    model_cfg = OmegaConf.to_container(
        OmegaConf.merge(model_conf, OmegaConf.create(runtime)), resolve=True
    )
    model_cfg.pop("checkpoint_dir", None)
    model = instantiate_from_dict(model_cfg, overrides={"device": device})
    if text_encoder is not None:
