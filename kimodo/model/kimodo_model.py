...
def _resolve_multiprompt_inputs(
    prompts: str | list[str],
    num_frames: int | list[int],
    num_samples: Optional[int],
) -> tuple[list[str], list[int], int, bool]:
    """Validate sequential-prompt dimensions independently of batch size.

    ``len(prompts)`` is the number of temporal segments, whereas
    ``num_samples`` is the number of independently generated sequences.  They
    are orthogonal quantities and must never be used to expand one another.
    """
    prompt_segments = [prompts] if isinstance(prompts, str) else list(prompts)
    if not prompt_segments:
        raise ValueError("multi-prompt generation requires at least one prompt")

    if isinstance(num_frames, int):
        frame_segments = [num_frames] * len(prompt_segments)
    else:
        frame_segments = list(num_frames)
        if len(frame_segments) != len(prompt_segments):
            raise ValueError(
                "multi-prompt num_frames must have one value per prompt segment "
                f"({len(frame_segments)} != {len(prompt_segments)})"
            )
    if any(
        not isinstance(value, int) or isinstance(value, bool) or value < 1
        for value in frame_segments
    ):
        raise ValueError("multi-prompt num_frames values must be positive integers")

    squeeze = num_samples is None
    resolved_samples = 1 if num_samples is None else num_samples
    if (
        not isinstance(resolved_samples, int)
        or isinstance(resolved_samples, bool)
        or resolved_samples < 1
    ):
        raise ValueError("num_samples must be a positive integer")
    return prompt_segments, frame_segments, resolved_samples, squeeze


def _resolve_batched_constraints(constraint_lst: Optional[list], num_samples: int) -> list[list]:
    """Normalize shared or per-sample constraints to one list per generated sample."""
    if not constraint_lst:
        return [[] for _ in range(num_samples)]
    if isinstance(constraint_lst[0], list):
        if len(constraint_lst) != num_samples:
            raise ValueError(
                "per-sample constraint_lst must have one list per generated sample "
                f"({len(constraint_lst)} != {num_samples})"
            )
        return [list(sample_constraints) for sample_constraints in constraint_lst]
    shared_constraints = list(constraint_lst)
    return [list(shared_constraints) for _ in range(num_samples)]
...
class Kimodo(nn.Module):
    ...
    def _multiprompt(
        ...
        prompts, num_frames, num_samples, tosqueeze = _resolve_multiprompt_inputs(
            prompts, num_frames, num_samples
        )
        bs = num_samples
        ...
        if len(texts) > 1:
            if (
                not isinstance(num_transition_frames, int)
                or isinstance(num_transition_frames, bool)
                or num_transition_frames < 1
            ):
                raise ValueError("num_transition_frames must be a positive integer")
            if num_transition_frames > min(num_frames):
                raise ValueError(
                    "num_transition_frames cannot exceed the shortest multi-prompt segment "
                    f"({num_transition_frames} > {min(num_frames)})"
                )

        if constraint_lst is None:
            constraint_lst = []
        constraints_by_sample = _resolve_batched_constraints(constraint_lst, bs)
        ...
        for idx, (text, num_frame) in enumerate(zip(texts, num_frames)):
            ...
            constraint_lst_base = [
                ...
                for sample_constraints in constraints_by_sample
            ]  # this moves constraints temporally but not spatially
                    ...
                    for bi in range(bs):
                        seg_constraints[bi].extend(
                            [c.crop_move(current_frame - nb_transition_frames,
                             ...
                             for c in constraints_by_sample[bi]]
                        )
